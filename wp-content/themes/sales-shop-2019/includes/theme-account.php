...
function ss_render_account_seller_reports_loop($coupons, $pagination_var_name = null, $pagination_per_page = null)
{
    $page_num = null;
    if (isset($pagination_var_name)) {
        $page_num = filter_input(INPUT_GET, $pagination_var_name, FILTER_VALIDATE_INT);
    ...
    for ($i = $coupons_from; $i < $coupons_to; $i++) {
        $order_item = $coupons[$i];
        echo ss_render_account_seller_report($order_item);
    }
